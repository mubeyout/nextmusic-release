...
const reloadServerData = async () => {
    ...
    const configPath = process.env.CONFIG_PATH || node_path_1.default.join(process.cwd(), 'config.js');
    if (node_fs_1.default.existsSync(configPath)) {
        try {
            ...
            log4js_1.startupLog.info('Config.js re-loaded and merged.');
        }
        catch (err) {
            log4js_1.startupLog.error('Failed to reload config.js:', err.message);
        }
    }
    ...
    const httpServer = node_http_1.default.createServer(async (req, res) => {
        ...
        if (pathname.startsWith('/api/')) {
                        ...
                        try {
                            ...
                            const handleFinalUpdate = () => {
                                ...
                                if (customMusicDir !== undefined)
                                    user.customMusicDir = String(customMusicDir).trim();
            ...
            if (pathname === '/api/user/verify' && req.method === 'POST') {
                void readBody(req).then(body => {
                    try {
                        ...
                        const user = global.lx.config.users.find(u => u.name === username);
                        const ok = !!user && (user.password.startsWith('$2') ? require('bcryptjs').compareSync(password, user.password) : user.password === password); // [P1 安全债]bcrypt 校验,旧明文兑底
                        if (ok) {
            ...
            if (pathname === '/api/user/library/artists' && req.method === 'POST') {
                ...
                void readBody(req).then(body => {
                    try {
                        ...
                        if (!node_fs_1.default.existsSync(libDir))
                            node_fs_1.default.mkdirSync(libDir, { recursive: true });
                        node_fs_1.default.writeFileSync(node_path_1.default.join(libDir, 'artists.json'), JSON.stringify(parsed, null, 2), 'utf-8');
                        res.writeHead(200, { 'Content-Type': 'application/json' });
                        res.end(JSON.stringify({ success: true }));
                    }
                    catch (e) {
            ...
            if (pathname === '/api/user/library/albums' && req.method === 'POST') {
                ...
                void readBody(req).then(body => {
                    try {
                        ...
                        if (!node_fs_1.default.existsSync(libDir))
                            node_fs_1.default.mkdirSync(libDir, { recursive: true });
                        node_fs_1.default.writeFileSync(node_path_1.default.join(libDir, 'albums.json'), JSON.stringify(parsed, null, 2), 'utf-8');
                        res.writeHead(200, { 'Content-Type': 'application/json' });
                        res.end(JSON.stringify({ success: true }));
                    }
                    catch (e) {
            ...
            if (pathname === '/api/music/url' && req.method === 'POST') {
                ...
                // [Gate 2026-09-14] 默认身份改为 token 所属用户(登录门已挡匿名);具名用户仍需验证匹配
                let verifiedUsername = (0, exports.verifyUserAuth)(req) || 'open';
            ...
            if (pathname === '/api/config') {
                ...
                if (req.method === 'GET') {
                    const config = {
                        ...
                        configFilePath: process.env.CONFIG_PATH || node_path_1.default.join(process.cwd(), 'config.js'),
                    };
                    res.writeHead(200, {
                        'Content-Type': 'application/json',
                        'Cache-Control': 'no-cache, no-store, must-revalidate'
                    ...
                    void readBody(req).then(body => {
                        try {
                            ...
                            if (newConfig['proxy.all.address'] !== undefined)
                                global.lx.config['proxy.all.address'] = newConfig['proxy.all.address'];
                            ...
                            node_fs_1.default.writeFileSync(configPath, configContent);
