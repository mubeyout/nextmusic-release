...
exports.fetchRadios = exports.fetchRadioSongs = exports.fetchSongsByGenre = exports.fetchPlaylistSongs = exports.fetchPlaylistsByGenre = exports.fetchGenres = void 0;
...
const fetchGenres = async () => {
    try {
        ...
        const { body } = await request_1.httpFetch(url.toString(), {
        ...
        const filteredGroups = categories.filter((c) => targetGroups.includes(c.categoryGroupName));
        const allItems = [];
 ...
 * 获取某流派标签下的热门歌单
 */
const fetchPlaylistsByGenre = async (categoryId, size = 20) => {
    try {
...
exports.fetchPlaylistsByGenre = fetchPlaylistsByGenre;
/**
 * 获取歌单详情 (歌曲列表)
 */
const fetchPlaylistSongs = async (dissid) => {
    try {
        ...
        if (!cd)
            return { name: '未知歌单', list: [] };
        return {
            name: cd.dissname,
            list: (cd.songlist || []).map((s) => ({
 ...
 */
const fetchSongsByGenre = async (categoryId, size = 20) => {
    try {
        const playlists = await (0, exports.fetchPlaylistsByGenre)(categoryId);
        if (playlists.length === 0)
            return [];
        // 同时抓取前 3 个热门歌单
        const targetPlaylists = playlists.slice(0, 3);
        const allResults = await Promise.all(targetPlaylists.map((pl) => {
            const dissid = pl.id.replace('alb_tx_playlist_', '');
            return (0, exports.fetchPlaylistSongs)(dissid).catch(() => null);
        }));
        // 汇聚去重
...
exports.fetchSongsByGenre = fetchSongsByGenre;
/**
 * 获取电台歌曲 (使用最新接口)
 */
const fetchRadioSongs = async (radioId) => {
    try {
        const data = {
            radio: {
...
exports.fetchRadioSongs = fetchRadioSongs;
/**
 * 获取电台列表
 */
...
exports.fetchRadios = fetchRadios;
