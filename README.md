...
- **全能第三方客户端** —— 一套 App 直连 **12 种自托管音乐服务**：Emby / Jellyfin / Plex / Navidrome / Subsonic / 道理鱼 / 群晖 Audio Station / 飞牛音乐 / mStream / Songloft / Audiobookshelf / WebDAV（NAS/Alist）——家里现成的曲库，一处聚合浏览播放，播完回写播放统计
- **投播设备** —— DLNA / AirPlay 投屏到客厅音响、电视、智能音箱；手机选歌、大屏出声；蓝牙/音频路由多输出切换
- **无缝接续** —— 手机听一半，回家音响接着放（同一局域网免费；跨网云接续属 Pro）
- **专业音效** —— 10 段 EQ + AutoEq 耳机校正 + ViPER-lite 低音/细节/响度，四端全链真生效（含浏览器）
- **音源治理** —— 自定义音源（LX 协议 / MusicFree 插件）+ 服务器共享音源多级容灾：多源排序自动回落、定时有效性检测（搜索+取链端到端）、连续失败自动禁用、上游新版本自动更新——音源挂了不用你管
- **自托管音乐云** —— 服务器免费自建（Docker 一条命令），歌缓存到自己的存储；多端下载并发可控（1-5 路）
- **告别会员** —— 本地播放 + 自定义音源 + 自己的服务器，不用再为听歌交月租
- **本地优先** —— 浏览/搜索/媒体库/自有音源全部匿名可用，登录只为云同步（歌单/设置/音源/缓存），数据不经任何第三方云

还有：黑胶沉浸播放页、逐字歌词与翻译/罗马音、歌单广场、播客聚合、音频可视化、匿名不强制注册、深色统一视觉、键盘快捷键+遥控器+触屏三套交互。
...
- **一次买断**：没有订阅、没有月租、没有广告（海外 $7.99 渠道筹备中）
...
NextMusic · 免费无广告，Pro ¥58 终身 · [☕ 爱发电](https://ifdian.net/a/nextmusic)
