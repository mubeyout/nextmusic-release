...
exports.default = {
    ...
    async getAlbumInfo(id) {
        const albumInfoRequest = await (0, util_1.createHttpFetch)('http://kmrserviceretry.kugou.com/container/v1/album?dfid=1tT5He3kxrNC4D29ad1MMb6F&mid=22945702112173152889429073101964063697&userid=0&appid=1005&clientver=11589', {
            method: 'POST',
            body: {
                appid: 1005,
        ...
        if (!albumInfoRequest)
            return Promise.reject(new Error('get album info failed.'));
    ...
    async getAlbumDetail(id, page = 1, limit = 200) {
        const albumList = await (0, util_1.createHttpFetch)(`http://mobiles.kugou.com/api/v3/album/song?version=9108&albumid=${id}&plat=0&pagesize=${limit}&area_code=0&page=${page}&with_res_tag=0`);
        if (!albumList.info)
            return Promise.reject(new Error('Get album list failed.'));
        let result = await (0, musicInfo_1.getMusicInfosByList)(albumList.info);
