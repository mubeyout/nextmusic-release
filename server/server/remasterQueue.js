...
exports.getStatus = exports.cancel = exports.start = exports.initialize = void 0;
...
const QUALITY_SET = new Set(QUALITY_ORDER);
const tasks = new Map();
let resolver = null;
...
const qualityRank = (quality) => QUALITY_ORDER.indexOf(quality);
const buildSongInfo = (item) => {
...
const waitForRetry = (attempt, signal) => new Promise((resolve, reject) => {
    const onAbort = () => {
        clearTimeout(timer);
...
const runTask = async (task, items, allItems) => {
    if (!resolver)
        throw new Error('洗版解析器尚未初始化');
    ...
    itemLoop: for (const item of items) {
        ...
        for (let attempt = 1; attempt <= MAX_REMASTER_ATTEMPTS; attempt++) {
            ...
            try {
                ...
                if (actualRank < 0)
                    throw new Error(`无法识别解析到的音质: ${actualQuality}`);
                ...
                if (task.isCustomDir) {
                    await customMusicManager.replaceCustomMusicItem(task.username, item, songInfo, resolved.url, actualQuality, task.controller.signal);
                }
                else {
                    await fileCache.replaceDownloadedMusicItem(task.username, item, songInfo, resolved.url, actualQuality, task.controller.signal);
                }
                ...
                if (task.controller.signal.aborted || err?.message === 'Aborted')
                    break itemLoop;
                lastError = err;
...
const start = async (username, targetQuality, filenames, explicitIsCustomDir) => {
    if (!resolver)
        throw new Error('洗版服务尚未就绪');
    if (!QUALITY_SET.has(targetQuality))
        throw new Error('不支持该目标音质');
    const current = tasks.get(username);
    if (current?.status === 'running')
        throw new Error('已有洗版任务正在运行');
    if (!Array.isArray(filenames))
        throw new Error('请选择需要洗版的歌曲');
    const selectedFilenames = [...new Set(filenames.filter((filename) => (typeof filename === 'string' && filename.length > 0 && filename.length <= 4096)))];
    if (selectedFilenames.length === 0)
        throw new Error('请至少选择一首歌曲');
    if (selectedFilenames.length > 10000)
        throw new Error('单次洗版最多选择 10000 首歌曲');
    ...
    void runTask(task, items, allItems).catch((err) => {
        task.status = task.controller.signal.aborted ? 'cancelled' : 'error';
        task.errorMsg = err?.message || '洗版任务异常终止';
        task.updatedAt = Date.now();
    });
    return (0, exports.getStatus)(username, 0, 0);
};
exports.start = start;
const cancel = (username) => {
...
const getStatus = (username, offset = 0, limit = 200) => {
    const task = tasks.get(username);
    if (!task)
        return {
            status: 'idle',
