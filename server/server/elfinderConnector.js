...
exports.ElFinderConnector = void 0;
exports.getSystemRoot = getSystemRoot;
...
class ElFinderConnector {
    ...
    // upload - 上传文件
    async cmdUpload(params) {
        // 这个会在路由中处理文件上传
        return { added: [] };
    }
    // file - 下载文件
    async cmdFile(params) {
        const target = this.decode(params.target);
        return { path: target };
    }
    // duplicate - 创建副本
    async cmdDuplicate(params) {
        const targets = Array.isArray(params['targets[]']) ? params['targets[]'] : [params['targets[]']];
        const added = [];
        for (const hash of targets) {
            if (!hash)
    ...
    async cmdZipdl(params) {
        ...
        const archive = new archiver_1.ZipArchive({ zlib: { level: 1 } });
        return new Promise((resolve, reject) => {
            output.on('close', () => {
                resolve({
                    zipdl: {
...
exports.ElFinderConnector = ElFinderConnector;
