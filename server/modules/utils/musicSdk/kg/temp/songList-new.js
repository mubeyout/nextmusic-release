...
exports.default = {
    ...
    async getListDetail(id, page) {
        ...
        if (id.startsWith('id_'))
            return this.getUserListDetailBySpecialId(id.replace('id_', ''), page);
    ...
    async getUserListDetailBySpecialId(id, page, tryNum = 0) {
        ...
        const { body } = await (0, request_1.httpFetch)(this.getSongListDetailUrl(id)).promise;
        let listData = body.match(this.regExps.listData);
        let listInfo = body.match(this.regExps.listInfo);
        if (!listData)
            return this.getListDetailBySpecialId(id, page, ++tryNum);
        let list = await (0, musicInfo_1.getMusicInfosByList)(JSON.parse(listData[1]));
        let name;
        let pic;
        if (listInfo) {
            name = listInfo[1];
    ...
    parseHtmlDesc(html) {
        ...
        if (index < 0)
            return null;
        return (0, index_1.decodeName)(afterStr.substring(0, index));
    },
    /**
    ...
    async getCollectionIdBySpecialId(specialId) {
        return (0, request_1.httpFetch)(`http://mobilecdnbj.kugou.com/api/v5/special/info?specialid=${specialId}`, {
            headers: {
    ...
    filterSongList(rawData) {
        return rawData.map(item => ({
            play_count: item.total_play_count || (0, index_1.formatPlayCount)(item.play_count),
            id: 'id_' + item.specialid,
            author: item.nickname,
            name: item.specialname,
            time: (0, index_1.dateFormat)(item.publish_time || item.publishtime, 'Y-M-D'),
    ...
    getSongList(sortId, tagId, page, tryNum = 0) {
        ...
        this._requestObj_list = (0, request_1.httpFetch)(this.getSongListUrl(sortId, tagId, page));
        return this._requestObj_list.promise.then(({ body }) => {
            if (!body || body.status !== 1)
                return this.getSongList(sortId, tagId, page, ++tryNum);
            return this.filterSongList(body.special_db);
        });
    },
    getSongListRecommend(tryNum = 0) {
        if (this._requestObj_listRecommend)
            this._requestObj_listRecommend.cancelHttp();
        if (tryNum > 2)
            return Promise.reject(new Error('try max num'));
        this._requestObj_listRecommend = (0, request_1.httpFetch)('http://everydayrec.service.kugou.com/guess_special_recommend', {
            method: 'post',
            headers: {
        ...
        return this._requestObj_listRecommend.promise.then(({ body }) => {
            if (body.status !== 1)
                return this.getSongListRecommend(++tryNum);
    ...
    async getUserListInfoByCollectionId(id) {
        if (!id || id.length > 1000)
            return Promise.reject(new Error('get list error'));
        if (this.collectionIdListInfoCache.has(id))
            return this.collectionIdListInfoCache.get(id);
        ...
        return (0, util_1.createHttpFetch)(`https://mobiles.kugou.com/api/v5/special/info_v2?${params}&signature=${(0, util_1.signatureParams)(params, 'web')}`, {
            headers: {
                mid: '1586163242519',
    ...
    async getUserListDetailByCollectionId(id, page = 1, limit = 300) {
        ...
        return (0, util_1.createHttpFetch)(`http://pubsongs.kugou.com/v2/get_other_list_file?${params}&signature=${(0, util_1.signatureParams)(params, 'android')}`, {
            headers: {
                'User-Agent': 'Android10-AndroidPhone-11589-201-0-playlist-wifi',
            },
        }).then(body => {
            if (!body.info)
                return Promise.reject(new Error('Get list failed.'));
            const songList = this.filterListByCollectionId(body.info);
            return {
                list: songList || [],
                page,
                ...
                info: {
                    ...
                    desc: listInfo.desc,
                    author: listInfo.userName,
                    play_count: (0, index_1.formatPlayCount)(listInfo.playCount),
                },
            };
        });
    },
    ...
    filterListByCollectionId(rawData) {
        ...
        rawData.forEach(item => {
            if (!item)
                return;
            if (ids.has(item.hash))
                return;
            ids.add(item.hash);
            const types = [];
            const _types = {};
            item.relate_goods.forEach(data => {
                let size = (0, index_1.sizeFormate)(data.size);
                switch (data.level) {
                    case 2:
            ...
            list.push({
                singer: (0, utils_1.formatSingerName)(item.singerinfo, 'name') || (0, index_1.decodeName)(item.name).split(' - ')[0].replace(/&/g, '、'),
                name: (0, index_1.decodeName)(item.name).split(' - ')[1],
                albumName: (0, index_1.decodeName)(item.albuminfo.name),
                albumId: item.albuminfo.id,
                songmid: item.audio_id,
                source: 'kg',
                interval: (0, index_1.formatPlayTime)(parseInt(item.timelen) / 1000),
                img: null,
                lrc: null,
                hash: item.hash,
                otherSource: null,
    ...
    async getUserListDetailByCode(id, page = 1) {
        ...
        const codeData = await (0, util_1.createHttpFetch)('http://t.kugou.com/command/', {
            method: 'POST',
            headers: {
                'KG-RC': 1,
        ...
        switch (codeInfo.type) {
            case 2:
                ...
                break;
            case 3:
                return album_1.default.getAlbumDetail(codeInfo.id, page);
        }
        if (codeInfo.global_collection_id)
            return this.getUserListDetailByCollectionId(codeInfo.global_collection_id, page);
        if (codeInfo.userid != null) {
            const songList = await (0, util_1.createHttpFetch)('http://www2.kugou.kugou.com/apps/kucodeAndShare/app/', {
                method: 'POST',
                headers: {
                    'KG-RC': 1,
                    'KG-THash': 'network_super_call.cpp:3676261689:379',
            ...
            let list = await (0, musicInfo_1.getMusicInfosByList)(songList || codeInfo.list);
            return {
                list,
                page: 1,
    ...
    async getUserListDetail3(chain, page) {
        const songInfo = await (0, util_1.createHttpFetch)(`http://m.kugou.com/schain/transfer?pagesize=${this.listDetailLimit}&chain=${chain}&su=1&page=${page}&n=0.7928855356604456`, {
            headers: {
        ...
        if (!songInfo.list) {
            if (songInfo.global_collection_id)
                ...
                return this.getUserListDetail4(songInfo, chain, page).catch(() => this.getUserListDetail5(chain));
        }
        let list = await (0, musicInfo_1.getMusicInfosByList)(songInfo.list);
    ...
    async getUserListDetailByLink({ info }, link) {
        ...
        while (total) {
            ...
            total -= limit;
            page += 1;
            tasks.push((0, util_1.createHttpFetch)(link.replace(/pagesize=\d+/, 'pagesize=' + limit).replace(/page=\d+/, 'page=' + page), {
                headers: {
                    'User-Agent': 'Mozilla/5.0 (iPhone; CPU iPhone OS 9_1 like Mac OS X) AppleWebKit/601.1.46 (KHTML, like Gecko) Version/9.0 Mobile/13B143 Safari/601.1',
                    Referer: link,
                },
        ...
        result = await (0, musicInfo_1.getMusicInfosByList)(result);
        // console.log(result)
        return {
            list: result,
            page,
    ...
    createGetListDetail2Task(id, total) {
        ...
        while (total) {
            ...
            tasks.push((0, util_1.createHttpFetch)(`https://mobiles.kugou.com/api/v5/special/song_v2?${params}&signature=${(0, util_1.signatureParams)(params, 'web')}`, {
                headers: {
                    mid: '1586163263991',
                    Referer: 'https://m3ws.kugou.com/share/index.php',
                    'User-Agent': 'Mozilla/5.0 (iPhone; CPU iPhone OS 11_0 like Mac OS X) AppleWebKit/604.1.38 (KHTML, like Gecko) Version/11.0 Mobile/15A372 Safari/604.1',
    ...
    async getUserListDetail2(global_collection_id) {
        ...
        const params = 'appid=1058&specialid=0&global_specialid=' + id + '&format=jsonp&srcappid=2919&clientver=20000&clienttime=1586163242519&mid=1586163242519&uuid=1586163242519&dfid=-';
        let info = await (0, util_1.createHttpFetch)(`https://mobiles.kugou.com/api/v5/special/info_v2?${params}&signature=${(0, util_1.signatureParams)(params, 'web')}`, {
            headers: {
                mid: '1586163242519',
                Referer: 'https://m3ws.kugou.com/share/index.php',
                'User-Agent': 'Mozilla/5.0 (iPhone; CPU iPhone OS 11_0 like Mac OS X) AppleWebKit/604.1.38 (KHTML, like Gecko) Version/11.0 Mobile/15A372 Safari/604.1',
        ...
        let list = await (0, musicInfo_1.getMusicInfosByList)(songInfo);
        ...
        return {
            ...
            info: {
                ...
                play_count: (0, index_1.formatPlayCount)(info.playcount),
            },
        };
    },
    async getListInfoByChain(chain) {
        ...
        const { body } = await (0, request_1.httpFetch)(`https://m.kugou.com/share/?chain=${chain}&id=${chain}`, {
            headers: {
    ...
    async getUserListDetailByPcChain(chain) {
        ...
        const { body } = await (0, request_1.httpFetch)(`http://www.kugou.com/share/${chain}.html`, {
            headers: {
    ...
    async getUserListDetail(link, page, retryNum = 0) {
        if (retryNum > 3)
            return Promise.reject(new Error('link try max num'));
        const requestLink = (0, request_1.httpFetch)(link, {
            headers: {
                'User-Agent': 'Mozilla/5.0 (iPhone; CPU iPhone OS 9_1 like Mac OS X) AppleWebKit/601.1.46 (KHTML, like Gecko) Version/9.0 Mobile/13B143 Safari/601.1',
                Referer: link,
            },
        ...
        if (typeof body == 'string') {
            ...
            if (body.includes('"albumid":'))
                return album_1.default.getAlbumDetail(body.replace(/^[\s\S]+?"albumid":(\w+)[\s\S]+?$/, '$1'), page);
            if (body.includes('"album_id":') && link.includes('album/info'))
                return album_1.default.getAlbumDetail(body.replace(/^[\s\S]+?"album_id":(\w+)[\s\S]+?$/, '$1'), page);
            if (body.includes('list_id = "') && link.includes('album/info'))
                return album_1.default.getAlbumDetail(body.replace(/^[\s\S]+?list_id = "(\w+)"[\s\S]+?$/, '$1'), page);
        }
        if (location) {
            ...
            if (location.includes('.html')) {
                if (location.includes('zlist.html')) {
                    ...
                    return this.getUserListDetail3(location.replace(/.+\/(\w+).html(?:\?.*|&.*$|#.*$|$)/, '$1'), page);
            }
    ...
    getListInfo(tagId, tryNum = 0) {
        if (this._requestObj_listInfo)
            this._requestObj_listInfo.cancelHttp();
        if (tryNum > 2)
            return Promise.reject(new Error('try max num'));
        this._requestObj_listInfo = (0, request_1.httpFetch)(this.getInfoUrl(tagId));
        return this._requestObj_listInfo.promise.then(({ body }) => {
            if (body.status !== 1)
                return this.getListInfo(tagId, ++tryNum);
            return {
    ...
    getList(sortId, tagId, page) {
        ...
        tasks.push(this.currentTagInfo.id === tagId
    ...
    getTags(tryNum = 0) {
        ...
        if (tryNum > 2)
            return Promise.reject(new Error('try max num'));
        this._requestObj_tags = (0, request_1.httpFetch)(this.getInfoUrl());
    ...
    search(text, page, limit = 20) {
        ...
        const url = encodeURI(`http://complexsearchretry.kugou.com/v1/search/special?${params}&signature=${(0, util_1.signatureParams)(params, 'android')}`);
        return (0, util_1.createHttpFetch)(url).then(body => {
            // console.log(body)
            return {
                list: body.lists.map(item => {
                    return {
                        play_count: (0, index_1.formatPlayCount)(item.total_play_count),
                        id: item.gid ? `gid_${item.gid}` : `id_${item.specialid}`,
                        author: item.nickname,
                        name: item.specialname,
                        time: (0, index_1.dateFormat)(item.publish_time, 'Y-M-D'),
                        img: item.img,
                        grade: item.grade,
                        desc: item.intro,
